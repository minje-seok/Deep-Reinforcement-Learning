...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        action = torch.tanh(self.action(x)) # * 10
...
class A2C(nn.Module):
    ...
    def train_net(self):
        ...
        next_Q = self.critic(s_prime, self.actor(s_prime)).detach()
        target_Q = r + gamma * next_Q * (1 - done) # target
        critic_loss = F.smooth_l1_loss(target_Q, current_Q) # advantages.pow(2).mean()

        # Actor loss
        predicted_a = self.actor(s)
        actor_loss = -self.critic(s, predicted_a).mean()
...
def main():
    ...
    model = A2C(env.observation_space.shape[0], action_dim=1)
    print_interval = 20
    print_score = 0.0
    scores = []
    reward_sum = 0
